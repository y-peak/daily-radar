...
### 论题卡：`data/thesis.json`

自选股表格回答"今天涨跌多少"，**回答不了"当初买它的理由还成不成立"**。
后者才是决定拿不拿的东西，而它恰恰是涨跌幅看不出来的 —— 所以单开一节。

每只股票可选地挂一张卡，key 用 secid（**独立文件**，不入仓库，和名单分开管）：
...
| 字段 | 作用 | 不填会怎样 |
|---|---|---|
| `statement` | 一句话论题 | 页面标"缺一句话论题" |
| `pillars[].status` | `on_track` / `behind` / `pending` | 认不出的值**一律按待验证算**（不猜） |
| `risks` | 什么情况下我认错 | ⭐ **判为"不可证伪"，页面标出来** |
| `exit` | 触发就走，不临场找理由 | 标"缺退出条件" |
| `catalysts` | 什么时候会知道答案 | 无（14 天内进"催化临近"，过期的单独标"已过"） |

三条不肯让步的设计：

* ⭐ **不可证伪的论题不算论题。** 写不出"什么情况下我认错"，它就不是论题、是口号 ——
  `risks` 为空会被 `validate()` 判为不合格并**印在页面上**，
  而不是宽容地渲染一张看起来完整的卡。
* ⭐ **走弱优先于达标。** 只要有一条支柱 `behind`，判词就是"需要重估"，
  不会被"另外两条还达标"平均掉 —— 论题失效通常就是从一条支柱塌掉开始的。
* ⭐ **90 天规则。** 超过 90 天没更新（或根本没写更新日期）→ 提示"该更新了"。
  一张半年没动过的卡，信息量等于没有。

论题卡也进**当日快照**（`collect` 原样写一份），所以历史页面显示的是
"当时我是怎么想的"，不会跟着文件改。没有论题卡时整节不渲染。

---

## 大模型的护栏（`radar/core/prompts.py`）

四条提示词（新闻导读 / 仓库总结 / 简介翻译 / 每日简报）以前各写各的"不许编造"，
写法各不相同、也没人检查是否真的生效。现在**收成一个文件**，理由是
"同一条规则写两遍，迟早漂" —— 和 `sources.py` 里那条关于 `_secid_of_cn` 的注释同一个道理。

| 纪律 | 我们这边的真实风险 |
|---|---|
| **外部文本不可信** | 新闻标题、仓库 README、英文简介都是**第三方原文**，直接拼进提示词 → 一条写着"忽略以上要求"的标题就能改写整段输出 |
| **数字必须有来源** | 模型爱"补"一个看起来合理的百分比。查不到出处的数字比没有数字更坏，所以宁可写 `[无来源]` |
| **结构化数据优先** | 模型会拿训练记忆补行情，而我们手里明明有当天真数据 |
...
# 输入里有别人写的字 → 用这个，并把那段文本夹进边界
system = prompts.system_for_untrusted("你是财经简讯编辑……")
user   = f"下面是快讯：\n{prompts.wrap_untrusted('标题A\n标题B', '今日财经快讯标题')}"

# 输入全是我们自己的结构化数字 → 用这个
system = prompts.system_for_numbers("你是每日复盘助手……")
```

⭐ 边界（`<<<外部内容开始>>>` … `<<<外部内容结束>>>`）不是装饰：模型对
"这段是引用"和"这段是指令"的区分，几乎完全靠这种显式标记；没有边界时，
一段标题和提示词本身在 token 层面长得一模一样。

⚠️ 改这个文件 = 改**所有**调用点的缓存键（`LLM._ckey` 用 system+user 做 key），
所以改完那一轮会重新花一次模型调用。这是预期行为。

自测在第 21 段：边界在内容**之前**出现、注入样例留在边界内、
自家事实留在边界**之外**、四个调用点都真的接上了。
...
全网快讯一天几千条，全搬过来又是一个没人看的瀑布流。这个页面的全部设计就围绕
**"怎么把它变窄"**：按主题分组、每组有名额上限、命中的自选股单独置顶。

来源两个，都是公开接口、都不需要 key：

| 源 | 接口 | 它多给了什么 |
|---|---|---|
| 华尔街见闻快讯 | `api-one.wallstcn.com/apiv1/content/lives`（按频道取） | 5 个频道：A股 / 全球 / 美股 / 港股 / 外汇 |
| 新浪财经 7×24 | `zhibo.sina.com.cn/api/zhibo/feed` | `tag[].name` 当分类；`ext.stocks[]` **自带关联标的** |

实测排除的（记在这里省得下次再试）：财联社要签名（`errno 10012`）、newsnow 公共实例 403、
雪球要 cookie、东财快讯返回的是 HTML。

### 自选股命中：三路，且刻意不对称

「这条跟我的票有没有关系」靠三条路，从硬到软：

1. **源自带的关联标的** —— 新浪自己标的，不是我们拿文字瞎撞，最硬
2. **正文里的 6 位代码** —— 快讯常直接写代码，精确无歧义
3. **名字匹配，但只认三个字以上的名字**

第 3 条那个限制不是偷懒：**「平安」「中国」「东方」这类词在财经新闻里到处都是**，
拿它去撞必然造出假信号 —— 一条"平安银行发布公告"被标成"和你的中国平安有关"，
比漏掉更糟，因为用户会**开始不信这个页面**。所以两字名只认代码。

反过来，"长数字串里不许切出假的 6 位码"也是同一件事：`6005190000123` 这样的订单号
中间正好藏着 `600519`，`(?<!\d)(\d{6})(?!\d)` 的环视就是为它写的。

### 分组规则：顺序即归属

规则写在 `radar/modules/news/rules.py` 的 `DEFAULT_RULES`（改自己的放
`data/news_rules.txt`，它不入库，所以个人偏好不会跟着仓库跑）：

```
[大盘]                 起一个新组
+A股 / 沪指 / 上证      必须命中其中之一（同组里多行 + 会合并成一个 OR）
!打新 / 广告            命中任一即从本组剔除（优先级高于 +）
@12                    本组最多留 12 条
/停牌|退市/             整行以 / 开头结尾 → 当正则用
```

**一条新闻只属于一个组，按规则从上到下先到先得**，所以"更专的"必须写在前面。
「全球」排在「公司」前面就是为此：讲美联储 / 油价的稿子常常顺带提一句公司业绩，
先撞「公司」的话，全球大事会被塞进公司组里，**两个组一起失真**。

`@` 上限是必需的而不是装饰：不设上限的话「公司」组一天能收两百条。
「未归类」也有个小上限（`unclassified_limit`，默认 8）—— 它装的是没归进任何主题的
国际 / 社会新闻，一放就是几十条，整页的重点会被它冲掉。

### 几条不肯让步的设计

* **跨日期的状态必须在 `collect` 里定下来。** 去重合并、`fresh`（对比最近 3 天的 guid）、
  自选股命中、关键词分组，全都在采集时算好写进快照。`analyze` / 渲染**只读今天这一份**——
  不然半年后重建旧页面，分组和增量会跟着"当时的最近三天"变，"历史页面不可变"就破了。
  冒烟里直接对 `analyze` 的源码断言它不碰 `data_dir`。

* **拿不到数据不许覆盖好数据。** 两个源都挂 → 抛错、今天不写快照，页面继续显示昨天那份好的。
  只挂一个 → 照常出页面，但页面顶部**如实写出来**：不说的话用户会以为"今天真的没有相关新闻"，
  而真相是"我们没拿到"。

* **指数不许混进「关联标的」。** 新浪对指数用过两种写法：`si931071`，以及
  `sz399975` / `sh000300`（后者照样匹配个股的 `^sh|sz\d{6}$`）。号段不冲突，所以按市场精确判断：
  沪市个股一律 `6` 开头 → `sh000xxx` 必是指数；深市个股是 000/001/002/003/300/301 →
  `sz399xxx` 必是指数。漏了这条，页面上会给一条普通新闻挂上「证券公司」，
  读者会以为那是某只票。

* **来源计数要归一。** 条目上同时有 `sources`（内部键 `wscn`/`sina`）和 `src_labels`（展示名）。
  两边都往计数里塞的话，同一个源会被算两次 —— 页面上就出现「新浪财经 100 · 新浪财经 100」。

* **空标题的去重键必须是空串。** 标题规范化后为空时，如果照常算 sha1，所有无标题条目会拿到
  同一个常量 `sha1('')`，于是**被合并成一条**：页面表现为"少了几条"，而且合出来那条标题是空的，
  谁也看不出发生过什么。

* **大模型导读只是锦上添花。** 三句话说出"今天集中在哪几个方向"，规则做不了这件事。
  但它任何一步出问题都只是"今天没有导读"，**绝不连累列表**；没配 key 就安静跳过。
...
```python
# radar/modules/news/sources.py
def fetch_mysrc(http, limit=30) -> list[dict]:
    # 每条必须有：src / sid / title / text / url / ts / tags / symbols / src_label
    ...

# fetch_all() 里追一段独立的 try/except，失败的写进 errors，成功的 items += rows
# ⚠️ 顺手 total_raw += len(rows) —— 漏了它，页面上会出现"原始 150 条 → 合并 175 条"
#    这种自相矛盾的数字
```

新的 `src` 键记得加进 `module.py` 的 `_src_set()` 归一表，否则它会以内部键的名字上页面。

### 自测

```bash
python3 smoke_test.py      # 第 20 段，130+ 项
```

第 20 段存在的理由很直接：**「自选股命中」靠真实数据是验不出来的**。
上次跑通的当天，250 条快讯里一个自选股名字都没出现（`茅台` 全文 0 次），
于是"命中 0 条"既可能是功能坏了、也可能是真的没有 —— **分不清的测试等于没有测试**。
所以这一段用合成条目把三条匹配路径、去重合并、分组顺序、增量判定全部钉死，不联网。
